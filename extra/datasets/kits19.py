...
def iterate(val=True, shuffle=False, bs=1, roi_shape=(128,128,128)):
  files = get_val_files() if val else get_train_files()
  if val: bs = 1 # NOTE: "ignore" bs for val=True
  order = list(range(0, len(files)))
  if shuffle: random.shuffle(order)
  for i in range(0, len(files), bs): yield (*map(np.stack, zip(*[preprocess(file, val, roi_shape) for file in files[i:i+bs]])), )
