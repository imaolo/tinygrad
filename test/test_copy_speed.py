...
class TestCopySpeed(unittest.TestCase):
  ...
  def testCopyCPUto6GPUs(self):
    ...
    if not hasattr(dev, "device_ids"): raise unittest.SkipTest("computer doesn't have multiple devices of the same type")
    if len(dev.device_ids) != 6: raise unittest.SkipTest("computer doesn't have 6 GPUs")
    t = Tensor.rand(N, N, device="cpu").realize()
    print(f"buffer: {t.nbytes()*1e-9:.2f} GB")
    for _ in range(3):
